...
import org.openlmis.referencedata.util.messagekeys.PageableMessageKeys;
...
public class MvcInterceptor extends HandlerInterceptorAdapter {
  ...
  private void validate(Object target, Errors errors) {
    ...
    if (ctx.getPage() != null) {
      if (ctx.getSize() == null) {
        errors.rejectValue(SIZE_PARAM, PageableMessageKeys.ERROR_SIZE_NULL,
                PageableMessageKeys.ERROR_SIZE_NULL);
      } else if (ctx.getSize() < 1) {
        errors.rejectValue(SIZE_PARAM, PageableMessageKeys.ERROR_SIZE_NOT_POSITIVE,
                PageableMessageKeys.ERROR_SIZE_NOT_POSITIVE);
      }
    }
  }
}
