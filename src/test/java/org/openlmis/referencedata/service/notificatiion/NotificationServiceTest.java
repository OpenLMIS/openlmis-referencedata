...
package org.openlmis.referencedata.service.notificatiion;
...
import org.openlmis.referencedata.service.notification.MessageDto;
import org.openlmis.referencedata.service.notification.NotificationChannelDto;
import org.openlmis.referencedata.service.notification.NotificationDto;
...
public class NotificationServiceTest {
  ...
  public void shouldNotifyUser() throws Exception {
    ...
    verify(restTemplate)
        .postForObject(eq(new URI(NOTIFICATION_URL)), captor.capture(), eq(Object.class));

    assertEquals(
        singletonList("Bearer " + ACCESS_TOKEN),
        captor.getValue().getHeaders().get(HttpHeaders.AUTHORIZATION));
    assertTrue(
        EqualsBuilder.reflectionEquals(getNotificationRequest(user), captor.getValue().getBody()));
  }

  private NotificationDto getNotificationRequest(User user) {
    Map<String, MessageDto> messages = new HashMap<>();
    messages.put(
        NotificationChannelDto.EMAIL.toString(), new MessageDto(MAIL_SUBJECT, MAIL_CONTENT));
